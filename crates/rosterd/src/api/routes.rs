...
//! R16.2, the journal reads of R18, and the cross node proxy of R7.5. Success bodies are the
//! plain object; errors are `{error}` with 400 validation, 404 not found, 409 conflict or
//! suspended, 413 transcript too large, 429 resume limit (`retry_after_s`), 502 proxy failure,
//! 503 no swarm.
...
/// Reads a peer fans out to over the mesh (usage, the journal of R18), mounted on every
/// listener so `/swarm/usage`, `/swarm/journal` and `/swarm/{node_id}/journal` can fetch a peer's.
pub fn reads() -> Router<Arc<Node>> {
    Router::new().route("/usage", get(usage)).route("/journal", get(journal)).route("/sessions/{key}/journal", get(session_journal))
}
...
async fn handoff(captures: Captures, Body(body): Body<Value>) -> Result<Response, ApiError> {
    ...
    node.journal.action("handoff", Some(captures.key.clone()), captures.by(), json!({ "to": peer.name, "session_key": to["session_key"] }));
