...
async fn a_session_through_the_real_holder_survives_a_daemon_restart() {
    ...
    assert_eq!(two.view().last_recap.as_deref(), Some("again"));
...
fn node(dir: &Path, holders: &Path, bin: &Path, fake: &Path, tweak: impl FnOnce(&mut Config)) -> Arc<Runner> {
    ...
    Runner::new(config, roster)
}

fn start(harness: &str) -> StartSession {
    StartSession { harness: harness.into(), name: Some("r15".into()), ..Default::default() }
}
...
async fn suspend_then_a_prompt_resumes_under_a_new_key_until_the_limit() {
    ...
    // R15.3 item 1: the prompt resumes first; one call, a new key, the same session id.
    ...
    assert!(!files[0].suspended);
    // Resuming a live key is the record itself; resuming the old key follows the session id.
...
#[tokio::test]
async fn resume_on_prompt_off_refuses() {
    let Some((bin, fake)) = binaries() else { return };
    let dir = short_dir("b");
    let holders = dir.join("h");
    let runner = node(&dir, &holders, &bin, &fake, |c| c.runner.resume_on_prompt = false);
    let key = runner.start(start("fake")).await.unwrap().session_key;
...
async fn idle_timeout_suspends_and_a_reboot_restores_as_suspended() {
    ...
    let key_live = runner.start(start("fake")).await.unwrap().session_key;
...
async fn pi_without_the_extension_warns_and_runs_as_auto() {
    ...
    let mut fake_req = start("fake");
    fake_req.permission_policy = Some(PermissionPolicy::Attention);
