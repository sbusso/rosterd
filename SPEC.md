...
The swarm as one agent of the Agent Client Protocol. A session id on this agent is a roster key, so one connection reaches every headless session on every node; a client that wants to know what is behind the connection asks `session/list`.

`rosterd acp [--node NODE] [--harness H]`. ACP on stdin and stdout, JSON-RPC newline delimited, protocol version 1. `initialize` answers `loadSession: true` and `sessionCapabilities.list`; there is no authentication. `session/list` is every live session in the swarm, `sessionId` the key, `title` the name, `cwd`, `updatedAt` the last activity, `_meta.rosterd` the node, harness, lane, activity and mode; `cwd` in the params filters. `session/new` is POST /sessions on the node named in `_meta.rosterd.node`, else `--node`, else this one, with the harness in `_meta.rosterd.harness`, else `--harness`, else refused; `name`, `model`, `effort` and `permission_policy` in the same object go into the start; the answer carries the key as `sessionId` and the modes and config options the harness answered with. `session/load` takes a key or a display name as R14.1 resolves it, refuses an interactive session, replays the conversation so far as `session/update` notifications, `user_message_chunk` and `agent_message_chunk` one per message, then answers the modes and config options. Every other request or notification with a `sessionId` is relayed to that session; `_rosterd/nodes` is GET /swarm/nodes. Ids are remapped both ways: the client keeps its own, the agent keeps its own.

`GET /sessions/{key}/acp`. A websocket; text frames are JSON-RPC messages. The owner node bridges the runner: every `session/update` of the session goes down with `sessionId` set to the key, so do the `session/request_permission` and `elicitation/create` requests left pending, as the agent sent them, the ones pending at connect time first. Up: `session/prompt` runs a turn through the runner (the prompt's content blocks as sent, the text journaled) and answers the stop reason; `session/cancel` cancels; a response to a pending request answers it, a late one is dropped; `_rosterd/handshake` answers the harness's session/new or session/load result; `_rosterd/history` answers `updates`, the user's and the agent's messages read from the harness's transcript (R15.5 finds it; text only, tool calls, thoughts and what the harness injects around the user's words left out; empty when there is none; over the transcript limit is refused); any other `session/*` request goes to the agent with the harness's own session id and its answer comes back. Any other node relays the socket to the owner over the mesh. Many clients may hold one session: each sees every update and request, the first answer wins. An interactive session is 400: it is attached, R9, not driven.

Names. A `session_info_update` with a `title` names the record at rank `acp`: a name given at start or through `/name` stands, a scanned title yields.

## R21. Screen

`rosterd tui`. The roster on a terminal, from /swarm/snapshot every second and GET /sessions/{key} for each session that needs attention, on the socket only. One table: sessions not ended, waiting first, then active, idle, unknown, suspended last, each newest activity first; columns name (R14.1's display name), cwd basename, node, harness, age of activity, and state: `suspended`, else `permission TOOL SUMMARY` or `question MESSAGE` with the options numbered, else the activity and its event. Keys: up/down or j/k, enter opens, q quits; on the selected session `1`-`9` answer the first pending request with that option (a question: 1 accepts, 2 declines), `y`, `a`, `n` the option of kind allow_once, allow_always, reject_once (or id allow, allow_always, deny), `c` POST /cancel, `z` /suspend, `r` /resume, `s` DELETE. An interactive session opens nothing: the note names `rosterd attach KEY`.

An open session is its websocket of R20: `_rosterd/history` first, then every update, rendered as lines: a user message as `> text`, agent chunks joined into one line until anything else, a tool call as `⚙ title`, a plan as `plan done/total  current`, a title as `· title`, a pending request as `? title`, once per request id; thoughts and tool progress are not shown. The line under it is POST /prompt, or POST /answer with `{action: accept, content: {FIELD: text}}` when the first pending question's schema has exactly one string property. Letters type; only with a request pending and the line empty do `1`-`9`, `y`, `a` and `n` answer. Esc closes. The last line of the screen is the outcome of the last action or the last error, cleared by the next key.
